...
鼻/耳是否可见）。在受试者 01 的四段上，水平角与之相关 0.67–0.86，真转头（>20°）时
60–88% 能被 15° 阈值捕获，静止时误报 <1%；**垂直角相关较弱（0.28–0.75），俯仰类结论请谨慎**。
这个核对信号正在为全部 88 段生成，生成后主表的 `yaw_keypoint_vs_rotation_corr` 列会给出每段的核对值。
