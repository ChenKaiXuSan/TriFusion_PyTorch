...
def analyse_sequence(name: str, args, splits: dict, det_grid: dict) -> dict:
    ...
    angle_source = "sam3d_head_rotation" if yaw_rot is not None else "keypoints"
    primary_yaw, primary_pitch = (yaw_rot, pitch_rot) if yaw_rot is not None else (yaw_c, pitch_c)

    series = {}
    for key, raw in (("yaw_rel", primary_yaw), ("pitch_rel", primary_pitch), ("yaw_kpt", yaw_c), ("pitch_kpt", pitch_c),
                     ("yaw_abs", yaw_abs), ("pitch_abs", pitch_abs)):
        sm = smooth(raw)
    ...
    with open(out_csv, "w", encoding="utf-8") as fh:
        fh.write("frame_id,time_s,in_drive,yaw_deg,pitch_deg,n_views,ai_turn_horizontal,ai_turn_vertical,"
                 "label_horizontal,label_vertical,yaw_kpt_deg,pitch_kpt_deg,yaw_abs_deg,pitch_abs_deg\n")
        HV = {0: '', 1: 'right', -1: 'left'}
        VV = {0: '', 1: 'up', -1: 'down'}
        for i in range(T):
...
def main() -> None:
    ...
    try:
        ...
        ax.plot(t, y, lw=0.8, color="k", label="head yaw from SAM3D head rotation, 3 views fused (deg, + = right)")
        for s, e in runs(ai != ""):
