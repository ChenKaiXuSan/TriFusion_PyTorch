...
## 二·十、Drive&Act 域内自适应（2026-08-30）：残差修正在新机位上同样成立

设置（`eval/build_driveandact_cache.py`）：6 段密集片段（vp5/vp11/vp13 × 2 run，各 1800 帧），
逐视角 canonicalize；参考为官方 openpose_3d 的 hs10 关节，逐帧相似对齐到均值融合的 canonical
系（排除定义不一致的 neck → 9 关节监督/评估）；**留一受试者 3 折**，1500 步，同一脚本。
单位 m，逐序列均值；"wrists" 为该协议下的手部组（仅腕）。

| 方法（Drive&Act 域内） | MPJPE | PA | Δ vs mean (MPJPE/PA) | wrists | 权重 f/l/r |
|---|---|---|---|---|---|
...
结论：同一配方（三角化/多视角参考监督的残差修正，8K 参数）在新机位上用该机位自己的自动参考
训练后，PA 从 24.3 → 18.6 mm（−23%），而零样本迁移的残差模型是 36 mm——**残差修正是"机位内
无标注自适应"**，不是跨机位通用先验；学到的视角权重也随机位改变（本数据集 front 0.14–0.32，
Drive&Act front 0.42–0.51），说明视角可靠性本身就是机位属性。

新论文模型章节四条主张的证据链至此完整：① 免训练 canonicalized 融合跨机位零样本
23.1 mm；② 损坏感知门控（增强训练）坏视角权重 0.02–0.10、置零视角达/优于 oracle、不损
干净精度、可迁移；③ 机位内三角化监督残差修正：本数据集 −14%、Drive&Act −18～−23%，8K 参数；
④ 逐关节不确定性 Spearman 0.72。
