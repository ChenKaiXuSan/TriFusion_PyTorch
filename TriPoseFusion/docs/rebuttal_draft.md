...
- **修正代码重训（Pegasus 955575–79）的 ckpt 修正评估（fold-0 val，
  ...
  **结论：架构开关与 epoch 均无关，所有配置收敛到同一解。** 直接测量（fold-0 val
  4 个 batch、53,248 关节-帧）：c_full 的 ‖P_final − mean_v(P̄_v)‖ = **6.2 mm**
  （c_base 6.8 mm），其中 P_init = 均值（gate 精确均匀）、差值全部来自 TCN 残差；
  与中位数融合差 26 mm，但三者在 MPJPE 上不可区分（<1 mm）。协议对照
  （`eval_median_via_model_path.py`，中位数融合走模型行同一度量路径）：MPJPE 0.947 /
  PA 0.347，四组 0.412/0.252/0.332/1.582——与模型行逐位相同。
  **即：修正评估下，完整模型 ≡ canonicalized 均值融合（+≈6 mm 残差）。**
...
- per-joint 分解（fold-0 val 20 序列，逐序列均值再平均，52 空间分组：
  ...
  ⚠️ **协议警示（2026-08-28 22:30 更正）**：上表模型行与基线行的 GT canonicalize
  实现不同（模型行：模型端 torch canonicalizer；基线行：eval 脚本 numpy
  canonicalize_pose 含回退）。协议对照实验表明，**同一度量路径下中位数融合的四组
  = 0.412 / 0.252 / 0.332 / 1.582（非鲁棒 canonicalizer，= c_base 行）；鲁棒 canonicalizer
  下 = 0.414 / 0.265 / 0.340 / 1.576，MPJPE 0.946 / PA 0.347（= 旧 ckpt full 行
  0.414 / 0.265 / 0.340 / 1.580）**——即"躯干 0.340 全场最优"和
  "手部 1.580 全场最差"都是两套 canonicalize 协议之差（手部离锚点最远、对旋转轴
  最敏感，相差 0.23 m），**不是模型能力，两句均撤回**。同理同子集表中的
  "MPJPE −4.4%" 也是跨协议假象。Table 3 定稿必须让所有行走同一路径
  （建议：基线也经 eval_median_via_model_path 的度量路径，或模型输出经 numpy 路径）。

  **D 点讲法（更正后）**：
  1. 绝对误差由 42 个手部关节主导——**所有方法**手部/躯干误差比 ≈ 3.5–4.7×
     （手部占 52 关节的 81%，伪 GT 手部有效率仅 34%）；Drive&Act 标准 body 协议
     下同一管线为厘米级（I 点）。这是与 SOTA 数字量级差异的来源。
  2. 同协议下，主模型与 canonicalized 均值/中位数融合在躯干与手部均无可测差异
     （A 点：模型 ≡ 均值融合 + 6 mm）；学习型基线（E 点）在同 numpy 协议下相对
     固定融合躯干 −7%、手部 −4%，是本文范围内唯一可测的学习增益。
  3. 手部退化不再归因于模型；手部误差的主因是伪 GT 手部 66% 无效与单目手部估计
     噪声，与 19jf 的手部质疑衔接，修订版按躯干/手部分组报告并对手部单独讨论。
...
实测（重生成伪 GT，只读统计，`eval/pseudo_gt_view_participation.py`；52 关节子集、
17.15M 个 valid 关节-帧）：各视角参与三角化的比例 **front 8.1% / left 98.4% /
right 93.4%**（计数 1.40M / 16.88M / 16.02M，与论文 Table 2 完全一致——说明
重生成的参考可复现论文统计）；参与时的逐视角重投影误差 front 15.5 px /
left 10.0 / right 10.2。按关节组：front 对头部/肩颈的参与率仅 0.3%/0.4%，对手部
13.5%。**结论：伪 GT 实质上是左/右两视角的立体三角化，前视几乎不参与**——正文
须明说：(i) 前视预测的评估相对独立于参考；(ii) 参考的深度精度由 A 柱左右相机
基线决定；(iii) Table 2 的"前视 42 px P95"只覆盖前视偶尔参与的 8% 关节-帧。
...
### 5.8 X7：更严格的 PCK 阈值
修正评估已输出更严格阈值：主模型（旧 ckpt，fold-0 val）PCK@0.02 / 0.05 / 0.10 =
0.048 / 0.069 / 0.159；修正基线（88 序列均值）PCK@0.05 / 0.10 = 0.057–0.058 /
0.111–0.113（best single 0.056 / 0.113；fuse mean 0.058 / 0.111）。补入表格
（注明子集口径），并说明手部关节主导下严格阈值数值偏低（D 点分解）；同时把
PCK 从"secondary diagnostic"改为按躯干/手部分组报告。
